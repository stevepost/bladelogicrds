...
#     Allowed values are TYPICAL, HISTORY, HISTORY_O, HISTORY_RDR, RETENTION, CLEAN_DB,
#     CLEAN_DB_TL, CLEAN_SHARED_OBJECTS, CLEAN_FS, CLEAN_FS_TL, CLEAN_AGENT, CLEAN_ALL_AS
#     CLEAN_AS, CLEAN_REPEATER, CHECK_FS and FULL_CLEANUP. 
...
#   Required argument are: DURATION RETENTION_DAYS ROLE
#
...
#   No additional arguments
#
# FULL_CLEANUP - BladeLogic full cleanup job
#   No additional arguments
#
# -----------------------------------------------------------------------------
...
# JobRunEvent = 8% (60 min), AuditTrail = 8% (60 min), cleanupDatabase = 50% (360 min), hardDeleteAllSharedObjects = 17% (120 min), cleanupFileServer = 17% (120 min) 
TYPICAL_DEFAULT_DURATION_DIST=(JobRunEvent=8 AuditTrail=8 cleanupDatabase=50 hardDeleteAllSharedObjects=17 cleanupFileServer=17)
...
case $MODE in
  ...
  FULL_CLEANUP)	MESSAGE="BladeLogic full cleanup job"
				NAMESPACE="Delete performFullCleanupJob"
								ARGUMENT_NAMES=()
                                ARGUMENT_VALUES=()
				DO_LOOPS="false" ;;
...
then
  ...
  then
    ...
    getDuration JobRunEvent
    jreDuration=$?
    echo "Clean JobRunEvent data (max $jreDuration minutes)"
    cmdOutput=`blcli -n false Delete cleanupHistoricalData JobRunEvent "$RETENTION_DAYS" "$jreDuration" "$ROLE"`
    retvalue=$?
    ...
    checkIsFailed "$retvalue" "JobRunEvent"
    ...
    getDuration AuditTrail
    atDuration=$?
    echo "Clean AuditTrail data (max $atDuration minutes)"
    blcli -n false Delete cleanupHistoricalData AuditTrail "$RETENTION_DAYS" "$atDuration" "$ROLE"
    retvalue=$?

    checkIsFailed "$retvalue" "AuditTrail"
    ...
    # ignore the failure
    if [[ `echo "$cmdOutput" | grep "Retention policy is disabled"` != "" ]]
    then
      retvalue=0
    fi
